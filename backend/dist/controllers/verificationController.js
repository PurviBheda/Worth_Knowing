...
exports.getFlaggedReports = exports.overrideReport = exports.triggerVerification = exports.getVerificationReport = exports.VerificationStatus = void 0;
...
async function getInMemoryReport(articleId) {
    if (inMemoryReports[articleId])
    ...
    const mockArt = rssArticles.find((a) => a.id === articleId) || mockData_1.MOCK_ARTICLES.find(a => a.id === articleId);
    if (!mockArt)
        return null;
    ...
    else if (mockArt.title.toLowerCase().includes('superconductivity'))
...
const getVerificationReport = async (req, res, next) => {
    try {
        ...
        try {
            const report = await prisma_1.default.verificationReport.findUnique({
                where: { articleId },
                include: {
                    article: {
            ...
            if (!report) {
                ...
                const newReport = await (0, verificationService_1.verifyArticle)(articleId);
                if (!newReport) {
...
exports.getVerificationReport = getVerificationReport;
...
const triggerVerification = async (req, res, next) => {
    try {
        ...
        try {
            const report = await (0, verificationService_1.verifyArticle)(articleId);
            if (!report) {
...
const overrideReport = async (req, res, next) => {
    try {
        const articleId = parseInt(req.params.articleId, 10);
        if (isNaN(articleId)) {
            res.status(400).json({ error: 'Invalid article ID' });
        ...
        try {
            ...
            let report = await prisma_1.default.verificationReport.findUnique({ where: { articleId } });
            if (!report) {
                await (0, verificationService_1.verifyArticle)(articleId);
            }
            const updatedReport = await prisma_1.default.verificationReport.update({
                where: { articleId },
                data: {
            ...
            if (isConnectionError(err)) {
                ...
                const mockArt = mockData_1.MOCK_ARTICLES.find(a => a.id === articleId);
                if (mockArt) {
...
exports.overrideReport = overrideReport;
    ...
    try {
        ...
        try {
            const [reports, total] = await Promise.all([
                prisma_1.default.verificationReport.findMany({
                ...
                prisma_1.default.verificationReport.count({ where: { isFlagged: true } })
            ]);
            res.json({
                data: reports,
                pagination: {
            ...
            if (isConnectionError(err)) {
                ...
                const reportsList = await Promise.all(mockData_1.MOCK_ARTICLES.map(a => getInMemoryReport(a.id)));
                const flaggedList = reportsList
                    ...
                    article: {
                        id: r.articleId,
                        title: mockData_1.MOCK_ARTICLES.find(a => a.id === r.articleId)?.title || 'Mock Title',
                        publishedAt: mockData_1.MOCK_ARTICLES.find(a => a.id === r.articleId)?.publishedAt || new Date(),
                        source: { name: mockData_1.MOCK_ARTICLES.find(a => a.id === r.articleId)?.source.name || 'Mock Source' }
                    }
                }));
...
exports.getFlaggedReports = getFlaggedReports;
