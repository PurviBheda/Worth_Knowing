...
exports.getLiveRssArticlesFallback = getLiveRssArticlesFallback;
const prisma_1 = __importDefault(require("../prisma"));
const client_1 = require("@prisma/client");
const mockData_1 = require("../mockData");
const rss_parser_1 = __importDefault(require("rss-parser"));
let cachedRssArticles = [];
let lastRssFetch = 0;
...
function getCategoryPoolImage(category, seed = 0) {
    const pools = {
        'AI & Technology': [
...
async function fetchGNewsFallback(apiKey) {
    const categories = [
...
async function getLiveRssArticlesFallback() {
    const now = Date.now();
...
const getArticles = async (req, res, next) => {
    const { search, categoryId, categorySlug, sourceId, sentiment, sortBy = 'publishedAt', order = 'desc', page = '1', limit = '10' } = req.query;
    try {
        ...
        if (sentiment && typeof sentiment === 'string') {
            ...
            if (['POSITIVE', 'NEGATIVE', 'NEUTRAL'].includes(upperSentiment)) {
                whereClause.sentiment = upperSentiment;
            }
        }
        // Build Sort clause
        ...
        const [articles, totalCount] = await Promise.all([
            ...
            prisma_1.default.article.count({ where: whereClause })
        ]);
        ...
        if (isConnectionError(error)) {
            ...
            if (search && typeof search === 'string') {
                ...
                filtered = filtered.filter(a => a.title.toLowerCase().includes(queryText) ||
    ...
    try {
        ...
        try {
            ...
            if (isConnectionError(error)) {
                ...
                const article = list.find(a => a.id === id) || mockData_1.MOCK_ARTICLES.find(a => a.id === id);
                if (!article) {
                    res.status(404).json({ error: 'Article not found' });
                    return;
                }
...
const createArticle = async (req, res, next) => {
    try {
        const { title, summary, content, time, publishedAt, credibility, sentiment, imageId, categoryId, sourceId } = req.body;
        // Basic Validation
        ...
        try {
            ...
            const category = await prisma_1.default.category.findUnique({ where: { id: catId } });
            if (!category) {
                res.status(404).json({ error: 'Referenced category does not exist' });
                return;
            }
            // Verify source exists
            const source = await prisma_1.default.source.findUnique({ where: { id: srcId } });
            if (!source) {
                res.status(404).json({ error: 'Referenced source does not exist' });
                return;
            }
            // Parse Sentiment
            let artSentiment = client_1.Sentiment.NEUTRAL;
            if (sentiment && typeof sentiment === 'string') {
                const upperSentiment = sentiment.toUpperCase();
                if (['POSITIVE', 'NEGATIVE', 'NEUTRAL'].includes(upperSentiment)) {
                    artSentiment = upperSentiment;
                }
            }
            // Create article
            ...
            if (isConnectionError(error)) {
                ...
                const category = mockData_1.MOCK_CATEGORIES.find(c => c.id === catId) || mockData_1.MOCK_CATEGORIES[0];
                const source = mockData_1.MOCK_SOURCES.find(s => s.id === srcId) || mockData_1.MOCK_SOURCES[0];
                const simulatedArticle = {
                    ...
                    publishedAt: publishedAt ? new Date(publishedAt) : new Date(),
                    credibility: credibility !== undefined ? parseInt(credibility, 10) : 100,
                    sentiment: (sentiment ? sentiment.toUpperCase() : 'NEUTRAL'),
                    imageId: imageId || null,
                    categoryId: catId,
                    sourceId: srcId,
                    category,
...
const updateArticle = async (req, res, next) => {
    try {
        ...
        try {
            // Check existence
            const article = await prisma_1.default.article.findUnique({ where: { id } });
            if (!article) {
                res.status(404).json({ error: 'Article not found' });
                return;
            }
            ...
            if (sentiment !== undefined && typeof sentiment === 'string') {
                const upperSentiment = sentiment.toUpperCase();
                if (['POSITIVE', 'NEGATIVE', 'NEUTRAL'].includes(upperSentiment)) {
                    updateData.sentiment = upperSentiment;
                }
            }
            ...
            if (isConnectionError(error)) {
                ...
                const original = mockData_1.MOCK_ARTICLES.find(a => a.id === id) || mockData_1.MOCK_ARTICLES[0];
                const updated = {
                    ...original,
                ...
                updated.category = mockData_1.MOCK_CATEGORIES.find(c => c.id === updated.categoryId) || updated.category;
                updated.source = mockData_1.MOCK_SOURCES.find(s => s.id === updated.sourceId) || updated.source;
                res.json(updated);
                return;
            }
            throw error;
