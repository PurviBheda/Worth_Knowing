...
exports.processPendingArticles = processPendingArticles;
...
// ── Prompt Builder ────────────────────────────────────────────────────────────
function buildArticlePrompt(title, content) {
...
function buildDailyBriefPrompt(articles) {
    const articleList = articles.map((a, i) => `[${i + 1}] ID:${a.id} | Category: ${a.category} | Credibility: ${a.credibility}/100
Title: ${a.title}
...
async function summarizeArticle(articleId) {
    try {
        ...
        // Upsert AiSummary
        const aiSummary = await prisma_1.default.aiSummary.upsert({
            where: { articleId },
            create: {
                articleId,
                headlineSummary,
        ...
        try {
            const { fetchUnsplashPhotoId } = await Promise.resolve().then(() => __importStar(require('./ingestionService')));
        ...
        if (tags.length > 0) {
            for (const tagName of tags) {
                ...
                if (!normalizedTag)
                    continue;
                await prisma_1.default.articleTag.upsert({
 ...
 */
async function generateDailyBrief(date = new Date()) {
    try {
        ...
        const brief = await prisma_1.default.dailyBrief.upsert({
 ...
 */
async function processPendingArticles(limit = 50, concurrency = 3) {
    const pending = await prisma_1.default.article.findMany({
    ...
    for (let i = 0; i < pending.length; i += concurrency) {
        ...
        for (const result of results) {
            if (result.status === 'fulfilled' && result.value) {
                processed++;
                if (result.value.flaggedForReview)
                    flagged++;
            }
            else {
                failed++;
...
async function getAiStats() {
    const [totalArticles, totalSummarized, flaggedCount, sentimentCounts, latestBrief] = await Promise.all([
        ...
        prisma_1.default.aiSummary.count(),
        prisma_1.default.aiSummary.count({ where: { flaggedForReview: true } }),
        prisma_1.default.aiSummary.groupBy({
            by: ['sentiment'],
            _count: { sentiment: true }
        }),
        prisma_1.default.dailyBrief.findFirst({
            orderBy: { date: 'desc' },
    ...
    return {
        ...
        sentimentBreakdown: sentimentCounts.reduce((acc, row) => {
