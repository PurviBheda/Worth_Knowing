...
// ── Prompt Builder ────────────────────────────────────────────────────────────
function buildVerificationPrompt(targetArticle, comparisonArticles) {
    const comparisonText = comparisonArticles.map((a, i) => `[Comparison Article #${i + 1}] (ID: ${a.id})
Title: ${a.title}
Source: ${a.source}
Published: ${a.date}
Snippet: ${a.summary}
...
function generateMockReport(title, source, articleId) {
    ...
    else if (title.toLowerCase().includes('superconductivity'))
...
async function verifyArticle(articleId) {
    try {
        ...
        try {
            ...
            const comparisonArticlesRaw = await prisma_1.default.article.findMany({
                where: {
                    id: { not: articleId },
        ...
        if (aiResult.duplicateTitlesDetected && aiResult.duplicateTitlesDetected.length > 0) {
            const dbDuplicates = await prisma_1.default.article.findMany({
                where: {
        ...
        const report = await prisma_1.default.verificationReport.upsert({
            where: { articleId },
            create: {
                articleId,
                status: finalStatus,
                ...
                supportingSources: aiResult.supportingSources,
                ...
                unsupportedClaims: aiResult.unsupportedClaims,
